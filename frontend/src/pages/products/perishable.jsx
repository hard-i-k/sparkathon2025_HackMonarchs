...
export default function PerishableProductsPage() {
  ...
  useEffect(() => {
    ...
    if (priceRange !== "all") {
      ...
      filtered = filtered.filter((product) => {
        if (max) {
          return product.dynamicPrice >= min && product.dynamicPrice <= max;
        }
    ...
    if (carbonFilter !== "all") {
      ...
      filtered = filtered.filter(
        (product) => product.carbonScore <= maxScore
      );
    }

    setFilteredProducts(filtered);
  }, [products, searchTerm, priceRange, carbonFilter]);
