...
export default function GroceryDashboard() {
  ...
  const upcomingEvents = [
    {
      ...
      date: "2024-01-25",
      type: "promotion",
      description: "Reduce prices by 15% for festival season",
      impact: "High"
    },
    {
      id: 2,
      ...
      date: "2024-01-20",
      type: "expiry",
      description: "Paneer and yogurt products expiring soon",
      impact: "Critical"
    },
    {
      id: 3,
      title: "Demand Spike - Bread & Pastries",
      date: "2024-01-22",
      type: "demand",
      description: "ML predicts 40% demand increase",
      impact: "Medium"
    },
    {
      id: 4,
      ...
      date: "2024-01-18",
      type: "competition",
      description: "Local competitors reduced prices by 10%",
      impact: "High"
    }
  ];
  ...
  const fetchProducts = async () => {
    ...
    setLoading(true);
    try {
      const response = await api.getGroceryProducts();
      let products = response.data;
      console.log('Fetched products:', products);
      // For each product, call the ML model API to get predicted price
      const updatedProducts = await Promise.all(products.map(async (product, idx) => {
        if (!product || !product.expiryDate || !product.dateOfManufacturing || !product.categoryId || !product.cityId || !product.dateAdded) {
          console.warn(`Skipping product at index ${idx} due to missing required fields:`, product);
          return product;
        }
        // Calculate days_to_expiry
        const expiry = new Date(product.expiryDate);
        const dom = new Date(product.dateOfManufacturing);
        const days_to_expiry = Math.ceil((expiry - dom) / (1000 * 60 * 60 * 24));
        // Extract dept_id correctly (e.g., FOODS_1 from FOODS_1_002)
        const dept_id = product.categoryId.split('_').slice(0, 2).join('_');
        const payload = {
          days_to_expiry,
          dept_id,
          city: product.cityId,
        ...
        console.log(`Requesting ML price for product at index ${idx}:`, payload);
        let bestPrice = product.bestPrice;
        try {
          const mlRes = await api.predictGroceryPrice(payload);
          console.log(`ML response for product at index ${idx}:`, mlRes.data);
          if (mlRes.data && mlRes.data.status === 'success' && mlRes.data.data && typeof mlRes.data.data.predicted_price === 'number') {
            bestPrice = mlRes.data.data.predicted_price;
          } else {
            console.warn(`ML API did not return a valid price for product at index ${idx}:`, mlRes.data);
          }
        } catch (err) {
          console.error(`Error predicting price for product at index ${idx}:`, err, product);
        }
        return { ...product, bestPrice };
      }));
      setProducts(updatedProducts);
      calculateStats(updatedProducts);
      console.log('Updated products with predicted prices:', updatedProducts);
    } catch (error) {
      console.error('Error fetching products or predicting prices:', error);
    } finally {
      setLoading(false);
      console.log('Finished fetching and updating products.');
    }
  };
  ...
  const handleEditPrice = async (product) => {
    ...
    try {
      const response = await api.getGroceryBestPrice(product._id);
  ...
  const handleUpdatePrice = async () => {
    ...
    setPriceLoading(true);
    try {
      const response = await api.updateGroceryPrice(editingProduct._id, newPrice);
    ...
    <Layout>
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Grocery Dashboard</h1>
            <p className="text-gray-600 mt-1">AI-Powered Product Management & Analytics</p>
          </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-900">${stats.totalValue.toFixed(2)}</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-indigo-600">Real-time ML pricing with 95% accuracy</p>
                </div>
              </div>
              <div className="flex items-center space-x-3">
                <BarChart3 className="h-5 w-5 text-indigo-600" />
                <div>
                  <p className="font-medium text-indigo-900">Demand Analysis</p>
                  <p className="text-sm text-indigo-600">Seasonal trends and demand forecasting</p>
                </div>
              </div>
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-indigo-600">Optimized FOODS_1_001 to FOODS_3_004</p>
                </div>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent>
            {products.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-500">Add your first grocery product to get started.</p>
              </div>
            ) : (
